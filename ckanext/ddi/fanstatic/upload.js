 ...
 *
 */
this.ckan.module('file-upload', function($) {
  return {
    ...
    initialize: function () {
      ...
      this.label_location = $('label[for="field-' + options.field_url + '"]');
      // determines if the resource is a data resource
      this.is_data_resource = this.options.is_data_resource;
      ...
      $('label[for="field-' + options.field_upload + '"]').text(options.upload_label || this._('Image'));
      ...
      if (options.is_url) {
        ...
        this._updateUrlLabel(this.options.upload_label);
        ...
        this._updateUrlLabel(this._(options.upload_label));
      } else {
